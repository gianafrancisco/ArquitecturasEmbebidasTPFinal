...
void CPUhighVector(void)
{
    _asm goto CPUInterrupt _endasm
}
...
//#pragma interruptlow CPUlowInterrupt
void CPUInterrupt(void)
{
    // Save processor registers
_asm
...
_endasm
    ...
    // If this is the first interrupt, save the current task's
    // stack pointer into the current task's OS_TCB:
    if(OSIntNesting == 1)
    {
        // Save the current task's stack pointer into the current task's OS_TCB:
        //   OSTCBCur->OSTCBStkPtr = Stack Pointer;
